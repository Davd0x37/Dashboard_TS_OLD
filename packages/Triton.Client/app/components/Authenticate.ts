...
class Authenticate extends Triton {
      ...
      <section class="container">
        <div class="tab login__tab">
          <form onsubmit="return false;">
            ...
            <input type="submit" id="login_submit" class="btn flex" value="${
              this.lang.Authenticate.loginLink
            }">
          </form>
        </div>
        <div class="tab register__tab">
          <form onsubmit="return false;">
            ...
            <input type="submit" id="register_submit" class="btn flex" value="${
              this.lang.Authenticate.registerLink
            }">
          </form>
        </div>
      </section>
  ...
  public mounted() {
    ...
    fromEvent(
      $(".authenticate > .container .tab #login_submit")!,
      "click"
    ).subscribe({
      next: async () => this.authenticateUser(this.login, this.password)
    });
    fromEvent(
      $(".authenticate > .container .tab #register_submit")!,
      "click"
    ).subscribe({
      next: async () =>
        this.registerUser(this.login, this.password, this.email, this.avatar)
    });
  }
  ...
  private async authenticateUser(login: string, password: string) {
    if (login.length !== 0 && password.length !== 0) {
      ...
      if (res) {
        ...
        success(`${this.lang.Messages.welcome} ${res.User.Login}!`, () =>
          App.go("/")
        );
      } else {
